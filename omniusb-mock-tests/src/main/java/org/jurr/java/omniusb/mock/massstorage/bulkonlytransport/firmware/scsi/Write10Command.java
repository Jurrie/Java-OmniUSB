...
public class Write10Command implements RBCCommand
{
	public static final byte OPERATION_CODE = 0x2A;

	private final int logicalBlockAddress;
	private final short transferLength;

	public Write10Command(final byte[] buffer)
	{
		if (buffer.length != getCommandLength())
		{
			throw new IllegalArgumentException("Write(10) command must be " + getCommandLength() + " bytes long but was " + buffer.length);
		}

		if (buffer[0] != OPERATION_CODE)
		{
			throw new IllegalArgumentException("Invalid command code for Write(10) command");
		}

		if (buffer[9] != 0)
		{
			throw new IllegalArgumentException("Control byte must be zero for Write(10) command");
		}
