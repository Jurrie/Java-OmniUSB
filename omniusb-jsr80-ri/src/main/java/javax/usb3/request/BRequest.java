...
public class BRequest
{
	...
	public BRequest(EDeviceRequest deviceRequest)
	{
		...
		bRequest = deviceRequest.getByteCode();
	}
	...
	{
		deviceRequest = EDeviceRequest.fromByteCode(bRequest);
	 ...
	 */
	public static byte getInstance(final EDeviceRequest deviceRequest)
	{
		return new BRequest(deviceRequest).getByteCode();
	}

	/**
	 * Get the Standard Device Requests instance for this BRequest.
	 * If the bRequest byte code is not known (for example it's class-specific), then this field is NULL. You can then still use {@link #getByteCode()} to get the actual byte code.
	 *
	 * @see #getByteCode()
	 */
	public EDeviceRequest getDeviceRequest()
	{
