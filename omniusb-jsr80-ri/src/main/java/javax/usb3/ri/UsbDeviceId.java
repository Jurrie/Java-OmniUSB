 ...
 */
// Based on org.usb4java:usb4java-javax:1.3.0#org.usb4java.javax.DeviceId
 ...
 */
public class UsbDeviceId implements Comparable<UsbDeviceId>
{
	/**
	...
	public UsbDeviceDescription getUsbDeviceDescription()
	{
		try
		{
			return UsbRepositoryDatabase.lookup(deviceDescriptor.idVendor(), deviceDescriptor.idProduct());
		}
		catch (Exception ex)
		{
			Logger.getLogger(UsbDeviceId.class.getName()).log(Level.FINE, "USB ID not found in the database: {0}:{1}", new Object[] { deviceDescriptor.idVendor(), deviceDescriptor.idProduct() });
			return null;
		}
	}
	...
	{
		int hash = 3;
		hash = 59 * hash + busNumber;
		hash = 59 * hash + deviceAddress;
		return hash;
	}
