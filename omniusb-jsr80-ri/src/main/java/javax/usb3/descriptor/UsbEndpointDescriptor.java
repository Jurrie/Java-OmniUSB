 ...
 */
// Based on org.usb4java:usb4java-javax:1.3.0#org.usb4java.javax.descriptors.SimpleUsbEndpointDescriptor
 ...
 */
public class UsbEndpointDescriptor extends AUsbDescriptor implements IUsbEndpointDescriptor
{
	/**
	 * The poll interval.
	 */
			...
			final byte bInterval)
	{
		...
		transferType = EDataFlowtype.fromByte(bmAttributes);
		synchronizationType = EEndpointSynchronizationType.fromByte(bmAttributes);
		usageType = EEndpointUsageType.fromByte(bmAttributes);
		interruptType = EEndpointInterruptType.fromByte(bmAttributes);
	}
	...
	{
		return bEndpointAddress.getByteCode();
	}
	...
	public int hashCode()
	{
		...
		hash += 67 * hash + bInterval;
		hash += 67 * hash + wMaxPacketSize;
		hash += 67 * hash + bmAttributes;
		hash += 67 * hash + Objects.hashCode(bEndpointAddress);
		return hash;
	}
	...
	public String toString()
	{
		return String.format(
				"USB Endpoint Descriptor:%n"
						+ "  bLength %18d%n"
						+ "  bDescriptorType %10d%n"
						+ "  bEndpointAddress      %s  EP %s %s%n"
						+ "  bmAttributes %13s%n"
						+ "    Transfer Type             %s%n"
						+ "    Synch Type                %s%n"
						+ "    Usage Type                %s%n"
				...
				endpointAddress().getEndPointNumber(),
				endpointAddress().getDirection(),
				bmAttributes() & 0xff,
				transferType,
				synchronizationType,
				usageType,
