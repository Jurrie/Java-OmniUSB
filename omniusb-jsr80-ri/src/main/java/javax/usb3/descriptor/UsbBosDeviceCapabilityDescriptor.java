 ...
 */
public class UsbBosDeviceCapabilityDescriptor extends AUsbDescriptor
{
	/**
	 ...
	 */
	public UsbBosDeviceCapabilityDescriptor(byte bDevCapabilityType, byte[] capabilityData)
	{
		super(EDescriptorType.DEVICE_CAPABILITY);
		capabilityType = EBosDeviceCapabilityType.fromByte(bDevCapabilityType);
		...
		case USB_20_EXTENSION:
			 ...
			 */
			linkPowerManagement = (capabilityData[0] & 0x02) != 0;
			break;
		case SUPERSPEED_USB:
			latencyToleranceMessages = (capabilityData[0] & 0x02) != 0;
			wSpeedsSupported = EDevicePortSpeed.speedSupported(capabilityData[1]); // Only first 4 bits are coded
			bFunctionalitySupport = EDevicePortSpeed.speedSupported(capabilityData[3]);
			bU1DevExitLat = capabilityData[4];
			wU2DevExitLat = ByteUtility.toShort(Arrays.copyOfRange(capabilityData, 5, 2));
			break;
		case CONTAINER_ID:
			/**
			 ...
			 * UUID.
			 */
			containerId = new String(capabilityData, 1, 16);
			break;
		case PLATFORM:
			/**
			 ...
			 */
			platformCapabilityUUID = new String(capabilityData, 1, 16);
			/**
			 * platformCapabilityData is a variable-length field containing data
			 * associated with the platform specific capability.
			 */
			platformCapabilityData = Arrays.copyOfRange(capabilityData, 17, capabilityData.length - 17);
			break;
		...
		case RESERVED:
			break;
		default:
			throw new AssertionError(capabilityType.name());
