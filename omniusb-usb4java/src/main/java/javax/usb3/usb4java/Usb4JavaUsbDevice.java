 ...
 */
public class Usb4JavaUsbDevice extends UsbDevice implements Usb4JavaAUsbDevice
{
	/**
	 ...
	 */
	protected final Usb4JavaUsbDeviceManager deviceManager;
	 ...
	 */
	public Usb4JavaUsbDevice(final Usb4JavaUsbDeviceManager deviceManager,
	...
	static IUsbDeviceDescriptor convertDescriptor(final DeviceDescriptor input)
	{
		return new UsbDeviceDescriptor(
				input.bcdUSB(),
				EUSBClassCode.fromByteCode(input.bDeviceClass()),
				input.bDeviceSubClass(),
				input.bDeviceProtocol(),
				input.bMaxPacketSize0(),
				input.idVendor(),
				input.idProduct(),
				input.bcdDevice(),
				input.iManufacturer(),
				input.iProduct(),
				input.iSerialNumber(),
				input.bNumConfigurations());
	}

	// TODO: Stuff below is duplicated in Usb4JavaUsbHub.
	...
	protected Usb4JavaUsbConfiguration doGetUsbConfiguration(final byte i) throws UsbPlatformException
	{
		...
		final int result = LibUsb.getConfigDescriptorByValue(device, i, configDescriptor);
		if (result < 0)
		{
	...
	{
		// Because we override doControlTransfer this method is never called.
	}
}