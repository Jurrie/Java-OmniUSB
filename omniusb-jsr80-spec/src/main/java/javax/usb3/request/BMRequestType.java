...
public class BMRequestType
{
	...
	public BMRequestType(final EEndpointDirection direction, final EType type, final ERecipient recipient)
	{
	...
	public BMRequestType(final byte bmRequestType)
	{
		direction = EEndpointDirection.fromByte(bmRequestType);
		type = EType.fromByte(bmRequestType);
		recipient = ERecipient.fromByte(bmRequestType);
	}
	 ...
	 * @return A Standard READ BMRequestType configuration encoded as a byte.
	 */
	public static byte getInstanceStandardRead(final ERecipient recipient)
	{
	...
	{
		// Developer note: Get a STANDARD Request Type where the data will flow DEVICE_TO_HOST and the control message recipient is the DEVICE.
	 ...
	 * @return A Standard READ BMRequestType configuration encoded as a byte.
	 */
	public static byte getInstanceStandardWrite(final ERecipient recipient)
	{
		// Developer note: Get a STANDARD Request Type where the data will flow DEVICE_TO_HOST and the control message recipient is the DEVICE.
	...
	public enum EType
	{
		 ...
		 */
		public static EType fromByte(final byte bmRequestType)
		{
	...
	public static enum ERecipient
	{
		...
		private ERecipient(final byte byteCode)
		{
			this.byteCode = byteCode;
		}
		 ...
		 * @return The bmRequestType Type
		 */
		public static ERecipient fromByte(final byte bmRequestType)
		{
