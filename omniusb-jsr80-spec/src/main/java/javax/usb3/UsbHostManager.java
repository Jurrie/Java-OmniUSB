...
public final class UsbHostManager
{
	...
	/**
	 * If there are more IUsbServices implementations found, this is the one that is used.
	 * It should be set to the canonical name of the implementation that should be used.
	 * By default this is NULL, meaning there should only be one implementation on the classpath. In case there are more, an error is thrown.
	 * If this value is set, but that implementation is not found, then that is NOT an error. We treat this case as if the value was NULL.
	 */
	private static String preferredImplementationCanonicalName = null;

	private UsbHostManager()
	{
	}

	public static void setPreferredImplementation(final String preferredImplementationCanonicalName)
	{
		UsbHostManager.preferredImplementationCanonicalName = preferredImplementationCanonicalName;
	}
	...
	public static IUsbServices getUsbServices() throws UsbException, SecurityException
	{
		synchronized (USB_SERVICES_LOCK)
		{
			if (null == usbServices)
			{
				final ServiceLoader<IUsbServices> loader = ServiceLoader.load(IUsbServices.class);
				final List<IUsbServices> allUsbServices = new ArrayList<>();
				for (IUsbServices srvc : loader)
				{
					allUsbServices.add(srvc);
					if (preferredImplementationCanonicalName != null && srvc.getClass().getCanonicalName().equals(preferredImplementationCanonicalName))
					{
						// Found the preferred implementation; we can halt the loop
						usbServices = srvc;
						break;
					}
				}
				...
				if (usbServices == null)
				{
					if (allUsbServices.isEmpty())
					{
						throw new UsbException("No USB services implementation found.");
					}
					else if (allUsbServices.size() == 1)
					{
						usbServices = allUsbServices.get(0);
					}
					else
					{
						final StringBuilder sb = new StringBuilder("Multiple USB services implementations found. Please set the preferred implementation to one of: ");
						sb.append(allUsbServices.get(0).getClass().getCanonicalName());
						for (int i = 1; i < allUsbServices.size(); i++)
						{
							sb.append(", ");
							sb.append(allUsbServices.get(i).getClass().getCanonicalName());
						}
						sb.append(".");
						if (preferredImplementationCanonicalName != null)
						{
							sb.append(" Please note that your current preferred implementation (\"" + preferredImplementationCanonicalName + "\") was not found.");
						}
	 ...
	 * <p>
	 * Set the productID to <code>-1</code> to capture all USB devices with the given vendor id.
	 * </p>
	 * <p>
	 * This method will start the search from the ROOT device.
	 * </p>
	 *
	 * @param vendorId The vendor ID to match.
	 * @param productId (Optional) The product id to match. Set to MINUS ONE (-1) to match all product IDs.
	 * @return A non-null ArrayList instance containing any matching IUsbDevice(s).
	 * @throws javax.usb3.exception.UsbException if the USB bus cannot be accessed (e.g. permission error)
	 * @since 3.1
	 */
	public static List<IUsbDevice> getUsbDeviceList(final short vendorId, final short productId) throws UsbException
	{
	 ...
	 * Set the productID to <code>-1</code> to capture all USB devices with the given vendor id.
	 *
	 * @param usbDevice The IUsbDevice to check. If null then a new recursive search from the ROOT device will be initiated.
	 * @param vendorId The vendor ID to match.
	 * @param productId (Optional) The product id to match. Set to MINUS ONE (-1) to match all product IDs.
	 * @return A non-null ArrayList instance containing any matching IUsbDevice(s).
	 * @throws javax.usb3.exception.UsbException if the USB bus cannot be accessed (e.g. permission error)
	 * @since 3.1
	 */
	public static List<IUsbDevice> getUsbDeviceList(final IUsbDevice usbDevice, final short vendorId, final short productId) throws UsbException
	{
	 ...
	 * Get a List of all devices that match the specified vendor and product id.
	 * <p>
	 * Set the productID to an empty list to capture all USB devices with the given vendor id.
	 * </p>
	 * <p>
	 * This method will start the search from the ROOT device.
	 * </p>
	 *
	 * @param vendorId The vendor ID to match.
	 * @param productId (Optional) A non-null list of product IDs to match. Provide an empty list to match all product IDs for the given vendor ID.
	 * @return A non-null ArrayList instance containing of any matching IUsbDevice(s).
	 * @since 3.1
	 */
	public static List<IUsbDevice> getUsbDeviceList(final short vendorId, final List<Short> productId) throws UsbException
	{
		return getUsbDeviceList(getUsbServices().getRootUsbHub(), vendorId, productId);
	}
	 ...
	 * @since 3.1
	 */
	public static List<IUsbDevice> getUsbDeviceList(final IUsbDevice usbDevice, final short vendorId, final List<Short> productId)
	{
		List<IUsbDevice> iUsbDeviceList = new ArrayList<>();
		/*
		 * A device's descriptor is always available. All descriptor field names and
